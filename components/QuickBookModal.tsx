            ...
            <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 relative text-center">
                    ...
                    <button 
                        ...
                        className="mt-6 w-full bg-rose-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-rose-600 transition-colors duration-300"
                    >
                        Fechar
                    </button>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 relative flex flex-col max-h-[90vh]">
                ...
                <form id="quick-book-form" onSubmit={handleSubmit} className="flex-grow overflow-y-auto space-y-4 pr-2">
                    <div>
                        <label htmlFor="client-name" className="block text-stone-600 mb-1 font-semibold">Nome do Cliente</label>
                        <input 
                            type="text" 
                            id="client-name" 
                            required 
                            ...
                            className="w-full px-4 py-2 border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-300" 
                        ...
                        <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                             {user.services.length > 0 ? user.services.map(service => (
                                <div
                                    ...
                                    className={`p-3 border rounded-lg cursor-pointer transition-all duration-200 ${selectedService?.id === service.id ? 'border-rose-500 bg-rose-50 ring-2 ring-rose-200' : 'border-stone-200 hover:bg-stone-100'}`}
                                >
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                {availableTimeSlots.map(time => (
                                    <button
                                        ...
                                        className={`p-2 rounded-lg text-sm transition-colors duration-200 ${selectedTime === time ? 'bg-rose-500 text-white' : 'bg-stone-100 hover:bg-stone-200 text-stone-700'} ${justSelectedTime === time ? 'animate-pop' : ''}`}
                                    >
                                        {time}
                                    </button>
                                ))}
                ...
                <div className="mt-auto flex-shrink-0 pt-4 border-t">
                    ...
                    <button 
                        ...
                        className="w-full bg-rose-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-rose-600 transition-colors duration-300 disabled:bg-rose-300"
                    >
